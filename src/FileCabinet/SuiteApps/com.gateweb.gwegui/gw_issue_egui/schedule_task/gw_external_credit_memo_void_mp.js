...
define(['./transactionDao/gw_credit_memo_dao',
	    ...
	    'N/transaction',
		'N/record'], function(creditMemoDao, sellerDao, evidenceStatusDao, apDocTypeOptionDao, transactionDao, StatusEmun, mainDao, gwMapUtils, gwPreferencesUtils, transaction, record) { 
	
	var voucherType = StatusEmun.CREDITMEMO_VOID_EXTERNAL_DOCUMENT.VOUCHERTYPE;  //EGUI or ALLOWANCE 
	var documentType = StatusEmun.CREDITMEMO_VOID_EXTERNAL_DOCUMENT.DOCUMENTTYPE;
	
	var completedEvidenceStatusValue = StatusEmun.CREDITMEMO_VOID_EXTERNAL_DOCUMENT.SUCCESS; //完成條件狀態	=>憑證已開立, 未進入關網系統
	var errorEvidenceStatusValue =StatusEmun.CREDITMEMO_VOID_EXTERNAL_DOCUMENT.ERROR;
    ...
    function getInputData(context) {
    	...
    	return creditMemoDao.getVoidTransactionIds(isOneWorldVersion);        
    }
    ...
    function summarize(context) { 
		...
		if (allSubsidiaryAry.length !=0) {        	
	        context.output.iterator().each(function(key, value) {
				...
				if (subsidiaryAry.length !=0) {
				    ...
				    if (voucherMainInternalId != -1) {
						...
						if (values.length != 0) {
							...
							completedEvidenceStatusValue = StatusEmun.CREDITMEMO_VOID_EXTERNAL_DOCUMENT.SUCCESS; 
						} 
			    ...
			    transactionDao.updateTransactionStatusAndLock(record.Type.CREDIT_MEMO, nsInternalId, evidenceIssueStatusId);
			    
		        transactionDao.voidTransaction(nsInternalId, transaction.Type.CREDIT_MEMO);
