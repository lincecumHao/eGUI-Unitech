...
define(['./transactionDao/gw_invoice_dao',
	    ...
	    'N/transaction',
		'N/record'], function(invoiceDao, sellerDao, evidenceStatusDao, apDocTypeOptionDao, transactionDao, StatusEmun, mainDao, gwMapUtils, gwPreferencesUtils, transaction, record) { 
    ...
    function getInputData(context) {
    	...
    	return invoiceDao.getVoidTransactionIds(isOneWorldVersion);        
    }
    ...
    function summarize(context) { 
		...
		if (allSubsidiaryAry.length !=0) {        	
	        context.output.iterator().each(function(key, value) {
				...
				if (subsidiaryAry.length !=0) {
				    ...
				    if (voucherMainInternalId != -1) {
						...
						if (values.length != 0) {
							...
							completedEvidenceStatusValue = StatusEmun.INVOICE_VOID_EXTERNAL_DOCUMENT.SUCCESS;
						}
				    } else {
			    ...
			    transactionDao.updateTransactionStatusAndLock(record.Type.INVOICE, nsInternalId, evidenceIssueStatusId);
			    
		        transactionDao.voidTransaction(nsInternalId, transaction.Type.INVOICE);
