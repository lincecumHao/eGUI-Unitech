...
define(['./daoFields/gw_common_dao', './daoFields/gw_document_status_emun', 'N/search'], function (gwCommonDao, StatusEmun, search) {
  ...
  function getOpenTransactionIds() {
	 ...
	 var evidenceStatusValue = StatusEmun.INVOICE_OPEN_EXTERNAL_DOCUMENT.OPEN; //憑證已開立, 外部代上傳  
  ...
  function getVoidTransactionIds() {
	 ...
	 var evidenceStatusValue = StatusEmun.INVOICE_VOID_EXTERNAL_DOCUMENT.OPEN; //憑證已作廢, 未進入關網系統  
  ...
  function getSearchTransactiosFilters(evidenceStatusValue, isLock) {	  
    ...
    searchFilters.push(['type', 'anyof', 'CustInvc']); //Invoice
    searchFilters.push('and');
    searchFilters.push(['recordtype', 'is', 'invoice']); 
    searchFilters.push('and');
